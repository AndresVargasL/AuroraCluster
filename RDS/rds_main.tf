...
resource "aws_db_subnet_group" "subgrp_aurora" {
  name          = "${join("-",tolist(["subgrp", var.workload, var.environment]))}"
...
resource "aws_kms_key" "aws_rds_key" {
  ...
  multi_region            = true
  tags                    = merge({ Name = "${join("-", tolist(["kmsrds", var.workload, var.environment]))}" })
}

resource "aws_kms_alias" "aws_rds_alias" {
  name          = join("-", tolist(["alias/kmsrds", var.workload, var.environment]))
...
resource "aws_rds_cluster" "cluster" {
  ...
  cluster_identifier      = join("-", tolist(["rdsclu", var.workload, var.environment,"auro"]))
  ...
  tags                    = merge({ Name = "${join("-", tolist(["rdsclu", var.workload, var.environment, "auro"]))}"})
}

resource "aws_rds_cluster_instance" "cluster_instances" {
  identifier          = join("-", tolist(["rdscluins", var.workload, var.environment, "auro"]))
  count               = 1
  ...
  tags                = merge({ Name = "${join("-", tolist(["rdscluins", var.workload, var.environment, "auro"]))}" })
}
